...
- **Work order documents** — optional file attachments (PDF/JPEG/PNG/WEBP, 5 MB/5-per-work-order caps), `WorkOrderDocuments.tsx`.
- **Login by email or phone** (`identifier` field), a mobile "Show/Hide password" toggle (`PasswordInput.tsx`) on every password field.
- Assignee contact details (email/phone), project customer contact details, work order completed-date columns, user mobile numbers on the entity/system Users pages — small display additions once the backend started returning the extra fields.
